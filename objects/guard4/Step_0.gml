...
while (switch_movement){
	...
	if ((randint == 1) && up_free) {
		...
		move_chance = 120;
		next_sprite = sGuardBackward4;
		switch_movement = false;
	} else if ((randint == 2) && down_free) {
		x_speed = 0;
		y_speed = movement_speed;
		move_chance = 120;
		next_sprite = sGuardForward4;
		switch_movement = false;
	} else if ((randint == 3) && left_free) {
		x_speed = -movement_speed;
		y_speed = 0;
		move_chance = 120;
		next_sprite = sGuardLeft4;
		switch_movement = false;
	} else  if ((randint == 4) && right_free) {
		x_speed = movement_speed;
		y_speed = 0;
		move_chance = 120;
		next_sprite = sGuardRight4;
		switch_movement = false;
	} else if (randint == 5) {
		x_speed = 0;
		y_speed = 0;
...
if ((x_speed == 0) && (y_speed == 0)){
	...
	if(x_speed > 0) {
		sprite_index = sGuardRight4
	} else {
		sprite_index = sGuardLeft4;
	}
} else if (abs(y_speed) > abs(x_speed)){
	if(y_speed > 0) {
		sprite_index = sGuardForward4;
	} else {
		sprite_index = sGuardBackward4;
	}
}
