...
while (switch_movement){
	...
	if ((randint == 1) && up_free) {
		...
		move_chance = 120;
		next_sprite = sGuardBackward2;
		switch_movement = false;
	} else if ((randint == 2) && down_free) {
		x_speed = 0;
		y_speed = movement_speed;
		move_chance = 120;
		next_sprite = sGuardForward2;
		switch_movement = false;
	} else if ((randint == 3) && left_free) {
		x_speed = -movement_speed;
		y_speed = 0;
		move_chance = 120;
		next_sprite = sGuardLeft2;
		switch_movement = false;
	} else  if ((randint == 4) && right_free) {
		x_speed = movement_speed;
		y_speed = 0;
		move_chance = 120;
		next_sprite = sGuardRight2;
		switch_movement = false;
	} else if (randint == 5) {
		x_speed = 0;
		y_speed = 0;
...
if ((x_speed == 0) && (y_speed == 0)){
	...
	if(x_speed > 0) {
		sprite_index = sGuardRightWalk2;
	} else {
		sprite_index = sGuardLeftWalk2;
	}
} else if (abs(y_speed) > abs(x_speed)){
	if(y_speed > 0) {
		sprite_index = sGuardForwardWalk2;
	} else {
		sprite_index = sGuardBackwardWalk2;
	}
}
