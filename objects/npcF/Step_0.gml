...
while (switch_movement){
	...
	if ((randint == 1) && up_free) {
		...
		move_chance = 80;
		next_sprite = sNpcfBackward;
		switch_movement = false;
	} else if ((randint == 2) && down_free) {
		x_speed = 0;
		y_speed = movement_speed;
		move_chance = 80;
		next_sprite = sNpcfForward;
		switch_movement = false;
	} else if ((randint == 3) && left_free) {
		x_speed = -movement_speed;
		y_speed = 0;
		move_chance = 80;
		next_sprite = sNpcfLeft;
		switch_movement = false;
	} else  if ((randint == 4) && right_free) {
		x_speed = movement_speed;
		y_speed = 0;
		move_chance = 120;
		next_sprite = sNpcfRight;
		switch_movement = false;
	} else if (randint == 5) {
		x_speed = 0;
		y_speed = 0;
...
if ((x_speed == 0) && (y_speed == 0)){
	...
	if(x_speed > 0) {
		sprite_index = sNpcfRightWalk;
	} else {
		sprite_index = sNpcfLeftWalk;
	}
} else if (abs(y_speed) > abs(x_speed)){
	if(y_speed > 0) {
		sprite_index = sNpcfForwardWalk;
	} else {
		sprite_index = sNpcfBackwardWalk
	}
}
